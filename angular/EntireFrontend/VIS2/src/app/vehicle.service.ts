...
cancelreqsetStatus(cancel_id:string):Observable<any>{
  return this.http.get('http://localhost:1216/vims/cancel/approve/'+cancel_id,{ responseType: 'text' as 'text'});
}
// cancel policy set status
cancelpolicyStatus(policy_id:string):Observable<any>{
  return this.http.get('http://localhost:1216/vims/cancel/approve/'+policy_id,{ responseType: 'text' as 'text'});
}
